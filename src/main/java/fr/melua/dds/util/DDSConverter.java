...
public class DDSConverter {
	...
	public static ByteBuffer convertToDxt1NoTransparency(ByteBuffer image, String mimeType) throws IOException {
		if (image == null) {
			String message = Logging.getMessage("nullValue.ByteBufferIsNull");
			Logging.logger().severe(message);
			throw new IllegalArgumentException(message);
	...
	public static ByteBuffer convertToDxt3(ByteBuffer image, String mimeType) throws IOException {
		if (image == null) {
			String message = Logging.getMessage("nullValue.ByteBufferIsNull");
			Logging.logger().severe(message);
			throw new IllegalArgumentException(message);
...
//        colorPoints[1].b = (colorPoints[1].b & 0xF8) | (colorPoints[1].b >> 5 );
	...
	protected static Color getColor888(int r8g8b8) {
		return new Color((int) (((long) r8g8b8) & 0xff0000) >> 16, (int) (((long) r8g8b8) & 0x00ff00) >> 8,
	...
	protected static void equalTransparentCase(TransparentColor[] colors, int[] extremaIndices, short value) {
		...
		/*
		 * else { // not transparent anywhere - it's all one colour, so we don't need to
